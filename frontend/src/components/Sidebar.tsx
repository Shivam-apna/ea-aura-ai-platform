...
const Sidebar: React.FC<SidebarProps> = ({ activeAgent, onSelectAgent, isCollapsed, onToggleCollapse }) => {
  ...
  useEffect(() => {
    console.log("Sidebar - Authenticated:", isAuthenticated, "Keycloak Loading:", keycloakLoading, "Client Roles:", clientRoles);
  }, [isAuthenticated, keycloakLoading, clientRoles]);
  ...
  const hasRequiredRole = (required: string[], userRoles: string[]) => {
    if (!isAuthenticated || keycloakLoading) return false; // Use keycloakLoading here
              ...
              return (
                <Button
                  ...
                  className={cn(
                    ...
                    isActive ? "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground shadow-md" : "text-foreground",
