...
const AdvancedDashboardLayout: React.FC<AdvancedDashboardLayoutProps> = ({
  ...
  const [chartTypes, setChartTypes] =
    useState<Record<string, string>>(initialChartTypes);
  const [chartColors, setChartColors] =
    useState<Record<string, string>>(initialChartColors);
  ...
  const [showSummaryMap, setShowSummaryMap] = useState<Record<string, boolean>>(
    {}
  ); // State for summary popup visibility
  const [individualSummaries, setIndividualSummaries] = useState<
    Record<string, string>
  >({});
  const [predictiveLoading, setPredictiveLoading] = useState<
  ...
  const [predictiveModalMetricKey, setPredictiveModalMetricKey] =
    useState<string>("");
  ...
  useEffect(() => {
    Object.entries(dynamicMetricGroups).forEach(([tab, metrics]) => {
      const visibleKeys = metrics
        .filter((metric) => !hiddenCharts.has(metric.key))
        .map((m) => m.key);
      setCardOrder((prev) => {
  ...
  const handleNextStepAnalysis = async (
    chartKey: string,
    chartLabel: string
  ) => {
    ...
    try {
      const chartType =
        chartTypes[chartKey] || charts[chartKey]?.plotType || "line";
      ...
      if (result && result.status === "success") {
        ...
        const errorMessage =
          result?.message ||
          result?.error ||
          "Failed to generate next step analysis";
  ...
  const handlePredictiveAnalysis = async (
    ...
    try {
      ...
      if (result) {
        ...
        const chartData =
          result.prediction_result || result.chart_data || result;
        const popupData = result.popup;

        // Check if this is successful prediction data
        if (
          chartData &&
          (chartData.status === "success" || (chartData.x && chartData.y))
        ) {
          ...
          const predictionMetadata =
            chartData._prediction_metadata ||
            chartData.prediction_metadata ||
            result.predictionmetadata;
        ...
        toast.error(
          "Failed to generate predictions. Check console for details."
        );
      }
    } catch (error) {
  ...
  return (
    <div className="relative min-h-screen w-full">
      {" "}
      {/* Removed max-w-[1500px] mx-auto px-6 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-2 w-full">
        {dynamicKpiKeys.map((kpi, idx) => {
          ...
          const isUsingDynamicColor =
            dynamicBgColor !== (kpi.bgColor || "#FFFFF");

          let textColor: string | undefined;

          if (
            dynamicBgColor.toLowerCase() === "#fff" ||
            dynamicBgColor.toLowerCase() === "white"
          ) {
          ...
          return (
            <ShadcnTooltip key={idx}>
              <TooltipTrigger asChild>
                <Card
                  style={{
                    ...
                    border: shouldShowStroke
                      ? `2px solid ${strokeColor}`
                      : "none",
                  }}
                  className={
                    "rounded-xl shadow-md transition-transform hover:scale-105 hover:shadow-lg p-0 overflow-hidden group min-h-[90px]"
                  }
                >
                      ...
                      {idx % 2 === 0 ? (
                        <BarChart2
                          className={cn(
                            ...
                            theme === "dark" ? "text-black" : "text-primary"
                          )}
                        />
                      ) : (
                        <LineChart
              ...
              <TooltipContent>
                <p>{colorInfo ? colorInfo.label : "Out of range / No data"}</p>
      ...
      <Tabs
        value={currentTab}
        onValueChange={handleTabChangeWithLog}
        ...
        <TabsContent key={currentTab} value={currentTab}>
          <div className="flex justify-end mb-2 gap-4 items-center">
            <Button
              ...
              disabled={metrics.every(
                (metric) => !hiddenCharts.has(metric.key)
              )}
            >
              Restore Graphs
            </Button>
              ...
              <PopoverContent className="w-auto p-4">
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-gray-500 pt-2 border-t">
                    {metrics.filter((m) => !hiddenCharts.has(m.key)).length} of{" "}
                    {metrics.length} graphs visible
                  </div>
                </div>
              </PopoverContent>
            </Popover>
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto flex gap-2 items-center">
                <Button
                  variant={columns === 1 ? "secondary" : "outline"}
                  size="icon"
            ...
            return (
              <>
                ...
                <PredictiveModal
                  ...
                  chartType={
                    predictiveModalData
                      ? chartTypes[predictiveModalMetricKey] ||
                        predictiveModalData.plotType
                      : undefined
                  }
                  chartColor={
                    predictiveModalData
                      ? chartColors[predictiveModalMetricKey] ||
                        predictiveModalData.marker?.color
                      : undefined
                  }
                />
