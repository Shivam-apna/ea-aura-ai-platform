...
const BrandIndex = () => {
  ...
  const { user } = useAuth(); // Add this line to destructure user
  ...
  const fetchData = async (prompt: string) => { // Modified to accept prompt as argument
    ...
    try {
      ...
      if (containsKpiRelated) {
        try {
          const existingKpiRaw = localStorage.getItem("brand_kpi_data");
          ...
          localStorage.setItem("brand_kpi_data", JSON.stringify(mergedKpi));
        } catch (e) {
